...
class UserDatabase {
    ...
    SignInResponse signIn(SignInRequest request) {
        ...
        if (db.getStatus() != UserStatus.NOT_ONLINE)
            return new SignInResponse(SignInResponse.SignInResponseType.ALREADY_ONLINE, username, System.currentTimeMillis());
    ...
    StartTalkResponse startTalk(StartTalkRequest request) {
        ...
        if (db.getStatus() == UserStatus.PVP_TALKING) {
            if (requestType == StartTalkRequest.StartTalkRequestType.PVE)
                return new StartTalkResponse(request.getUser(),
                        "START TALK ERROR: request for PVE, but PVP_TALKING",
                        System.currentTimeMillis());
            else
                return new StartTalkResponse(StartTalkResponse.StartTalkResponseType.ALREADY_CONNECTED,
                        request.getUser(), System.currentTimeMillis(), db.getAnother());
        }
        if (db.getStatus() == UserStatus.PVE_TALKING) {
            if (requestType == StartTalkRequest.StartTalkRequestType.PVP)
                return new StartTalkResponse(request.getUser(),
                        "START TALK ERROR: request for PVP, but PVE_TALKING",
                        System.currentTimeMillis());
            else
                return new StartTalkResponse(StartTalkResponse.StartTalkResponseType.ALREADY_CONNECTED,
                        request.getUser(), System.currentTimeMillis(), null);
        }
        if (db.getStatus() == UserStatus.WAITING) {
            return new StartTalkResponse(request.getUser(),
                    "START TALK ERROR: is PVP_WAITING",
                    System.currentTimeMillis());
        }
        ...
        if (requestType == StartTalkRequest.StartTalkRequestType.PVE) {
            ...
            for (String user : userDataList) {
                ...
                db2.setUser(user);
                if (db2.getStatus() == UserStatus.WAITING && !db2.getName().equals(userName)) {
                    find = true;
    ...
    EndTalkResponse endTalk(EndTalkRequest request) {
        ...
        if (db.getStatus() == UserStatus.PVE_TALKING) {
            ...
            db.setStatus(UserStatus.NOT_TALKING);
            db.push();
            return new EndTalkResponse(EndTalkResponse.EndTalkResponseType.PASSED, userName, System.currentTimeMillis(), "");
        }
        if (db.getStatus() != UserStatus.PVP_TALKING) {
